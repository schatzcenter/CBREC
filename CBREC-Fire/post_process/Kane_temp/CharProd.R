...
scens <- scenarios_matrix %>% 
        ...
        dplyr::select(ID, Silvicultural_Treatment, Burn_Type) 

unique(scens$Burn_Type)

unique(scenarios_matrix$Burn_Type)
...
for (i in 1:13) {
        ...
        df_out <- df_out %>%
                dplyr::select(x,y,Silvicultural_Treatment,Burn_Type, pile_char,char_fwd_residue,char_cwd_residue)%>%
                mutate(char = (pile_char + char_fwd_residue + char_cwd_residue) / 1000000 * 2.47105)# convert to Mg/ha
        ...
        # Create bar graph
            plots[[i]] <- ggplot(df_out) +
                    theme_classic() +
                    geom_boxplot(aes(x = Burn_Type, y = char)) +
...
q <- grid.arrange(
        arrangeGrob(grobs=lapply(plots, function(p) p + guides(colour=FALSE)), ncol=3, 
                    ...
                    left=textGrob("Char Production (Mg/ha)", gp=gpar(fontsize=15), rot=90)) 
)
q
ggsave("data/Post_process/charts/char_Silvic.png", dpi = 400, q, width = 12, height = 12)

write.csv(df_out,"data/Post_process/char_Silvic.csv")

df_out%>%
        group_by(Silvicultural_Treatment, Burn_Type)%>%
        summarize("Char"=mean(char))->charsum

char<-read_csv("data/Post_process/char_Silvic.csv", col_types = cols())

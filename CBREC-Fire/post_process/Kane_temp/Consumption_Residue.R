...
#scens <- split(scens, scens$Burn_Type)
...
for (i in 1:length(dfs)) {
        ...
        df_out <- df_out %>%
                dplyr::select(x,y,Burn_Type, total_foliage_exposed,total_fwd_exposed,total_cwd_exposed,total_fuel_consumed)%>%
                mutate(total_fuel = (total_foliage_exposed+total_fwd_exposed+total_cwd_exposed) / 1000000 * 2.47105,# convert to Mg/ha
                       total_cons = total_fuel_consumed/ 1000000 * 2.47105,
                       PerChange = total_cons / total_fuel *100)
     ...
     #   scenarioName <- ifelse(scenarioName != "Clearcut", gsub("0", "0%", scenarioName), scenarioName)
        ...
        ggplot(df_out) +
                theme_classic() +
                geom_boxplot(aes(x = Burn_Type, y = PerChange)) +

...
add_residue <- function(dt, timestep) {
        ...
        lookup_scattered <- fread("data/SERC/lookup_tables/limited-cases/scattered_in_field.csv", 
                                  verbose = FALSE)
        ...
        # update fuelbed
        # propfuel() relies on data.table order of op to work. For example, hun_hr_sound_pr calculates
        #       correctly because the value of hun_hr_sound is the original, before it is updated by
        #       addfuel(). Even though addfuel() is called before propfuel(), the original value
        #       of hun_hr_sound is used for both. This relies on the inherent order of operations of
        #       data.table. Note that this could break with future updates to data.table. The method
        #       for calculating the residue fractions (i.e. *_pr variables) should be updated so that
        #       they do not rely on this feature of data.table.
        ...
        #Add additional columns to store total exposed mass (residue + fuelbed) before passing through combustion.

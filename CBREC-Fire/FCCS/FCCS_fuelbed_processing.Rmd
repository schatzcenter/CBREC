...
add 0.01 to duff loading to examine if this is why some fuelbeds are not consuming litter.

```{r}
FCCS%>%
        mutate(duff_upper_loading  = case_when(duff_upper_loading ==0 ~ 0.01,
                                             TRUE~duff_upper_loading),
               duff_upper_depth  = case_when(duff_upper_depth ==0 ~ 0.01,
                                             TRUE~duff_upper_depth))->FCCS
```

...
calc_emissions <- function(dt, burn_type) {
        ...
        # 2019-06-25 Jerome: changed to use new *_initial variables that were added in add_residue()
        #       This allows the true pre-burn exposed residue mass to be reported
        dt[, ':=' (total_duff_exposed = (duff_upper_loading_initial * duff_upper_load_pr),
                   total_foliage_exposed = (litter_loading_initial * litter_loading_pr),
                   total_fwd_exposed = ((one_hr_sound_initial * one_hr_sound_pr) +
                                                (ten_hr_sound_initial * ten_hr_sound_pr) +
                                                (hun_hr_sound_initial * hun_hr_sound_pr)),
                   total_cwd_exposed = ((oneK_hr_sound_initial * oneK_hr_sound_pr) +
                                                (oneK_hr_rotten_initial * oneK_hr_rotten_pr) +
                                                (tenK_hr_sound_initial * tenK_hr_sound_pr) +
                                                (tenK_hr_rotten_initial * tenK_hr_rotten_pr) +
                                                (tnkp_hr_sound_initial * tnkp_hr_sound_pr) +
                                                (tnkp_hr_rotten_initial * tnkp_hr_rotten_pr)))]
        
        # define output data 
        # 2019-07-05 jerome: need to subtract char here in order for add_rx_residue() to use correct remaining residue
        # 2019-07-05 jerome: add all remaining piled material post-burn to oneK scattered size class
        out_dt <- dt[,list(x, 
                           y,
                           fuelbed_number, 
                           FCID2018, 
                           ...
                           hun_hr_sound = (hun_hr_sound - total_100 - char_100) * hun_hr_sound_pr,
                           oneK_hr_sound = ((oneK_hr_sound - total_OneK_snd - char_OneK_snd) * oneK_hr_sound_pr) + (pile_load - (flamg_pile + smoldg_pile + resid_pile) - pile_char),
                           oneK_hr_rotten = (oneK_hr_rotten - total_OneK_rot - char_OneK_rot) * oneK_hr_rotten_pr,
                           tenK_hr_sound = (tenK_hr_sound - total_tenK_snd - char_tenK_snd) * tenK_hr_sound_pr, 
                           tenK_hr_rotten = (tenK_hr_rotten - total_tenK_rot - char_tenK_rot) * tenK_hr_rotten_pr,
                           tnkp_hr_sound = (tnkp_hr_sound - total_tnkp_snd - char_tnkp_snd) * tnkp_hr_sound_pr,
                           tnkp_hr_rotten = (tnkp_hr_rotten - total_tnkp_rot - char_tnkp_rot) * tnkp_hr_rotten_pr,
                           pile_load = 0,
                           duff_upper_load_pr,
                           litter_loading_pr,
                           one_hr_sound_pr,

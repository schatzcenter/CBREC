...
# char for piles
# m_exp: material exposed pre-combustion
char_pile <- function(m_exp) {
        m_exp * 0.01
}
...
ccon_activity_fast <- function(dt, fm_type, days_since_rain, DRR, burn_type){
       ...
       # From consume source: DISCREPANCY b/t SOURCE and DOCUMENTATION here
        ...
        dt[, duff_depth := duff_upper_depth + duff_lower_depth] # where duff depth is calculated. A zero will cause an error in how litter is calcualted
        ...
        dt[, duff_reduction2 := duff_reduction * ((0.25 * duff_depth) + 0.375)] # if duff_depth is zero, duff_reduction(2) will be zero
        ...
        dt[, ffr_total_depth := duff_depth + litter_depth + lichen_depth + moss_depth] # moss and lichen may have values when duff and litter are zero
        dt[, duff_quotient := (duff_reduction / duff_depth)]
        dt[duff_depth == 0, duff_quotient := 0] # resolves NaN values from 0 duff depth
        dt[, calculated_reduction := duff_quotient * ffr_total_depth] # calculated_reduction will also be 0 since duff_quotient will be zero
        ...
        csd <- c(0.90, 0.10, 0.0)
        dt[, litter_reduction := ffr] # 0 value for ffr makes litter_reduction 0
        dt[litter_depth < ffr, litter_reduction := litter_depth] # if litter depth is 0 total litter becomes 0 later on
        dt[, ffr_errorflag := ffr - litter_reduction] # this should never be less than 0
        if(nrow(dt[(ffr_errorflag < 0) | is.na(ffr_errorflag), ]) > 0) stop(paste0("Error - Case ID = ",as.character(unique(dt[,ID])),": Negative or NaN ff reduction for litter found in calc_activity_fast()"))
        #how much was it reduced relative to the layer depth
        dt[, litter_proportional_reduction := litter_reduction / litter_depth] # This can return NaN values from 0/0
        dt[litter_depth <= 0.0, litter_proportional_reduction := 0] # turn NaN values to 0
        dt[, total_litter := litter_proportional_reduction * litter_loading] # this is where errors are propagated. If litter_proportional_reduction is zero there will be zero total litter to burn
        ...
        dt[duff_upper_depth < ffr, duff_reduction := duff_upper_depth] # only the upper is used to calculate these values
        dt[, ffr_errorflag := ffr - duff_reduction] # this should never be less than 0
        if(nrow(dt[(ffr_errorflag < 0) | is.na(ffr_errorflag), ]) > 0) stop(paste0("Error - Case ID = ",as.character(unique(dt[,ID])),": Negative or NaN ff reduction for duff found in calc_activity_fast()"))
        #how much was it reduced relative to the layer depth
        ...
        dt[, ':=' (char_100 = ifelse(total_100 + char_scat(total_100) > hun_hr_sound,
                                     ...
                                     char_scat(total_100)),
                   char_OneK_snd = ifelse(total_OneK_snd + char_scat(total_OneK_snd) > oneK_hr_sound,
                                          oneK_hr_sound - total_OneK_snd,
                                          char_scat(total_OneK_snd)),
                   char_OneK_rot = ifelse(total_OneK_rot + char_scat(total_OneK_rot) > oneK_hr_rotten,
                                          oneK_hr_rotten - total_OneK_rot,
                                          char_scat(total_OneK_rot)),
                   char_tenK_snd = ifelse(total_tenK_snd + char_scat(total_tenK_snd) > tenK_hr_sound,
                                          tenK_hr_sound - total_tenK_snd,
                                          char_scat(total_tenK_snd)),
                   char_tenK_rot = ifelse(total_tenK_rot + char_scat(total_tenK_rot) > tenK_hr_rotten,
                                          tenK_hr_rotten - total_tenK_rot,
                                          char_scat(total_tenK_rot)),
                   char_tnkp_snd = ifelse(total_tnkp_snd + char_scat(total_tnkp_snd) > tnkp_hr_sound,
                                          tnkp_hr_sound - total_tnkp_snd,
                                          char_scat(total_tnkp_snd)),
                   char_tnkp_rot = ifelse(total_tnkp_rot + char_scat(total_tnkp_rot) > tnkp_hr_rotten,
        ...
        dt[, ':=' (flamg_duff_residue = flamg_duff * duff_upper_load_pr, # if _pr is zero this will show zero flaming residue
